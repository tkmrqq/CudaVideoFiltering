...
CuDecode::CuDecode(CUcontext ctx, int width, int height)
...
void CuDecode::setFrameCallback(std::function<void(const std::vector<uint8_t> &, int, int, int)> cb) {
    frameCallback = std::move(cb);
}

bool CuDecode::init(cudaVideoCodec codec) {
    CUVIDPARSERPARAMS parserParams = {};
    parserParams.CodecType = codec;// Используем переданный кодек
    parserParams.ulMaxNumDecodeSurfaces = 8;
    parserParams.ulMaxDisplayDelay = 0;
    parserParams.pUserData = this;
    parserParams.pfnSequenceCallback = &CuDecode::HandleVideoSequence;
    parserParams.pfnDecodePicture = &CuDecode::HandlePictureDecode;
    parserParams.pfnDisplayPicture = &CuDecode::HandlePictureDisplay;
...
void CuDecode::decodePacket(const AVPacket *pkt) {
    if (!cuParser) {
...
void CuDecode::attachEncoderMux(NVENCEncoder *enc, MkvMuxer *mux) {
    this->encoder = enc;
    this->muxer = mux;
}
...
int CUDAAPI CuDecode::HandleVideoSequence(void *userData, CUVIDEOFORMAT *format) {
    CuDecode *self = reinterpret_cast<CuDecode *>(userData);
...
int CUDAAPI CuDecode::HandlePictureDecode(void *userData, CUVIDPICPARAMS *pic) {
    CuDecode *self = reinterpret_cast<CuDecode *>(userData);
    CUcontext oldCtx = nullptr;
    cuCtxPushCurrent(self->cuCtx);
...
int CUDAAPI CuDecode::HandlePictureDisplay(void *userData, CUVIDPARSERDISPINFO *dispInfo) {
    CuDecode *self = reinterpret_cast<CuDecode *>(userData);
    CUcontext oldCtx = nullptr;
    cuCtxPushCurrent(self->cuCtx);
